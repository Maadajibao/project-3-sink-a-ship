...
def random_point(size):
    ...
    and the length of the board(size) minus one.
...
def populate_board(board):
    """
    Will populate x with random_point
    and the same with y with ships from add_ship function.
    """
...
def valid_coordinates(x, y):
    ...
    try:
        ...
        if x < 0 or x > 4 or y < 0 or y > 4:
            raise ValueError("Your shot is out of bounds!", 
            "Choose a number within the board's range")
...
def make_guess(board):
    ...
    for a guess and checks if they are valid.
...
def run_game(computer_board, player_board):
    ...
    while True:
        ...
        if player_guess:
            ...
            # Check if the player hit a shit so that i can increment it
            
            if  "Boom" in player_result:  
                scores["player"] += 1
